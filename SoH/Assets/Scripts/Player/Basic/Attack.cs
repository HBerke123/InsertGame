...
public class Attack : MonoBehaviour
{
    ...
    private void Update()
    {
        if (((Time.time - th >= skillholdtime) || Input.GetMouseButtonUp(1)) && attackable && (th > 0))
        {
        ...
        else if (Input.GetMouseButtonDown(0) && attackable) {
            attackable = false;
            attacknum = 0;
    ...
    IEnumerator RegainCE(int gaincount)
    {
        for (int i = 0; i < gaincount; i++) { 
            yield return new WaitForSecondsRealtime(ceregaintime / 10);
